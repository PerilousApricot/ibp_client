...
int cmd_allocate(ibp_connect_context_t *cc, char **argv, int argc)
{
  ibp_depot_t depot;
  ...
  set_ibp_alloc_op(&op, &caps, size, &depot, &attr, timeout, NULL, cc);
...
int cmd_probe(ibp_connect_context_t *cc, char **argv, int argc)
{
  ibp_attributes_t attr;
  ...
  set_ibp_probe_op(&op, cap, &probe, timeout, NULL, cc);
  err = ibp_sync_command(&op);
  ...
  get_ibp_attributes(&attr, &duration, &rel, &type); 
  ctime_r(&duration, print_time); print_time[strlen(print_time)-1] = '\0';
...
int cmd_version(ibp_connect_context_t *cc, char **argv, int argc)
{
  ibp_depot_t depot;
  ibp_op_t op;
  char buffer[4096];
  ...
  set_ibp_version_op(&op, &depot, buffer, sizeof(buffer), timeout, NULL, cc);
