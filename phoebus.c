...
phoebus_t *global_phoebus = NULL;
  ...
  unsigned int phoebus_get_key(phoebus_t *p) { return(0); };
...
unsigned int phoebus_get_key(phoebus_t *p) 
{
...
void phoebus_print(FILE *fd)
{
  ...
  fprintf(fd, "[phoebus]\n");
  if (global_phoebus->p_count <= 0) return;

  fprintf(fd, "gateway = ");
  n = global_phoebus->p_count-1;
  for (i=0; i<n; i++) {
     fprintf(fd, "%s,", global_phoebus->path[i]);
  }
