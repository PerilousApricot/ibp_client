...
int main(int argc, char **argv)
{
  ...
  if (argc < 12) {
     ...
     printf("          [-sync] [-alias-source] [-alias-dest]\n");
  ...
  alias_source = 0;
  if (strcmp(argv[i], "-alias-source") == 0) { //** Check if we want alias allocs
     alias_source = 1;
     i++;
  }

  alias_dest = 0;
  if (strcmp(argv[i], "-alias-dest") == 0) { //** Check if we want alias allocs
     alias_dest = 1;
     i++;
  }
